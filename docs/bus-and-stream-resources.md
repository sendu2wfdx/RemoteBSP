...
固件侧现有第一条可编译竖切只实现与板卡无关的边界，不代表任一实体板卡已经
支持 I2C/SPI。`CONFIG_REMOTEBSP_BUS` 默认关闭；开启时，板级代码必须同时提供
...
验证均为 Ubuntu WSL 中的纯软件测试；G431、F103、F072 均尚未实现实体 I2C/SPI HAL，
也没有实体板卡电气、波形、事务恢复或时序结论。
