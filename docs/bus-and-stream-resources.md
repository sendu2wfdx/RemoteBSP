...
父总线忙和合同不匹配次数；同一隔离域还累计 MCU 明确返回的 Ok、NACK、Timeout、Busy、
Fault 与 LimitExceeded。快照按节点与资源 ID 稳定排序，并校验逐资源合计与全局合计；
节点失效只清除该节点的遥测，不污染其他节点。`toolbusd` 健康快照以 `Count` 单位发布
稳定扩展 ID `0x8008`～`0x800b` 的四项主机准入合计，以及 `0x800c`～`0x800e` 的
远端 NACK、Timeout、Fault 合计。远端分类只发生在线协议结果严格解码成功之后；未知或
未来状态保持透明转发、不猜测、不计入任何已知类别。

远端计数是 MCU 固件返回的事务语义证据，不是逻辑分析仪或示波器采集的实体波形证据，
也不能单独证明引脚电平、时序或实际从设备行为。主机本地准入产生的 Busy/LimitExceeded
仍只进入准入计数，不冒充远端结果；实体结论必须由统一板级环境与波形记录给出。

资源级当前状态通过版本 2 的总线遥测快照维护，并投影到版本 3 的 Runtime IPC 与
`remote-cli runtime-snapshot --json` 的 `bus_health` 数组。每项固定绑定
`(node_id, resource_id)`，包含最后一个明确远端状态、该结果发生时的 toolbusd 单调时钟
微秒值、当前连续失败数及历史连续失败峰值。成功的 `Ok` 表示软件观察到该资源恢复，
会把当前连续失败清零，但保留累计错误和历史峰值；节点代次失效则清除该节点全部状态，
不会继承到重连设备。单调时间只在同一 toolbusd 进程代次内可比较，不是 UTC 时间。
未知状态既不更新最后状态，也不改变连续失败和峰值。快照每项固定 28 字节、最多 128 项，
编码边界校验节点归属、重复资源、状态范围和 `current <= peak`，保持内存与 IPC 大小有界。

### 13.1 设备级恢复控制

I2C/SPI 设备恢复复用通用 `ResourceReset(resource_id)`，不增加绕过资源模型的总线私有
命令。调用者必须先持有目标设备的独占租约；无租约、租约属于其他会话或租约已超时均
返回 `AccessDenied`。MCU/Mock 只调用目标设备的 reset 回调：失败时保持该设备的
backend poison 和租约，成功时只清该设备状态并释放该设备租约，其他设备和父总线合同
不变，因此一个设备恢复不会串行清空或阻塞无关资源。

`toolbusd` 只有在节点代次未变化且收到明确 `Ok` 响应时，才把该资源 Runtime 当前连续
失败清零并把最后状态置为“尚无复位后结果”；累计计数和历史峰值继续保留。IPC 超时、
连接断开、错误响应或响应丢失都属于不确定结果，不更新恢复状态，也不会自动重试可能已
