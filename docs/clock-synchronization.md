...
`toolbusd --motion-max-clock-error-ns` 同时设置 ClockModel 的同步状态上限和运动组
冻结上限；默认仍为 100000 ns。两处必须保持一致，因为协调器不能使用已被模型标记为
`DEGRADED` 的估计。纯软件多进程回归可显式放宽该值以隔离宿主机调度抖动，但这种
配置只验证事务语义，不能作为实体总线同步精度证据。
...
Runtime 短缓存会用每个有效估计的当前 `sample_age_ns` 收紧实际命中期限：尚未陈旧的
快照最多缓存到样本年龄阈值边界，下一毫秒必须重新读取。已经带陈旧告警的快照仍可按
配置的短缓存合并并发读取，因为它没有隐藏质量退化。该规则只防止告警被缓存延迟，
不会在 Runtime 中自行外推或改写 toolbusd 时钟模型。
