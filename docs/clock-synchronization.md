...
的自由运行计数器，并把模型纳入节点在线、分配和启动代次生命周期。当前不定义线协议、
不发送同步报文、不修改 MCU 时钟，也不接触实体板。
...
当前发现和心跳协议尚不携带 `boot_epoch`，因此注册表不会从固件版本、计数器回绕或
主机重连中猜测代次。未来同步协议必须提供由 MCU 每次启动唯一生成或持久递增的可靠
值；在此之前这些 API 只用于纯软件接入和测试。
...
时钟算法和节点生命周期已经接入 `remotebsp_request_manager` 及 CMake 测试体系。后续
仍需要：

1. 同步协议落地后，由请求管理器记录主机发送/接收时间，MCU 返回接收/发送 tick，
   并提供可靠 `boot_epoch`，再调用 `register_clock_model()` 与 `add_clock_sample()`；
2. PREPARE/READY 只能使用 `host_to_node_time()` 的成功结果，并额外校验运动组内所有
   节点使用的启动代次和模型代数；
3. COMMIT 前固定所用模型代数和换算结果，不能让新样本改写已提交段；
4. 运行中节点离线、代次变化或误差越界时，由未来运动组状态机统一决定共同边界停机，
   本模块不自行发送停止命令。
