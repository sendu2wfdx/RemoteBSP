...
APP 默认选择外部 8 MHz HSE，系统、CAN 和运动定时器都以该晶振经 PLL 产生的
48 MHz 时钟为基准。FLY-D5 当前硬件基线没有声明 32.768 kHz LSE，因此不会为
低速晶振额外保留 PC14/PC15；这两个引脚仍按板卡既有步进槽接线使用。
...
以下引脚来自 Mellow 官方 Klipper 参考配置，作为Studio静态资源工程和智能运动
执行器的基线：
...
因此固件不会为了呼吸灯占用上述加热、风扇、步进或探针资源；由Studio工程
