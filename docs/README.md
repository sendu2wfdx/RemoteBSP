...
| 智能运动 | 第一阶段已实现、已测试 | Mock多轴、TIM2 compare调度、限位停机和遥测；TimeSync v1及跨板PREPARE/READY/COMMIT协调器/参与者Mock已完成纯软件闭环，守护进程接线与实体时序验收待实现 |
| TMC2209 | 第一阶段已实现、部分实测 | FLY-D5五路单线通信及五电机已实测，F103/G431待系统验收 |
| Studio | 构建阶段已实现 | 工程schema v2、冲突检查、I2C/SPI图形编辑、Mock可视化、确定性工程差异、`.config`与32线程构建；可生成中文接线表、资源摘要、稳定清单和SHA-256资料包，自动烧录回读待实现 |
...
| Runtime API | 只读竖切已实现、已测试 | 版本化REST接口及单次本地快照IPC；拓扑稳定、资源故障隔离、短缓存、新鲜度和HTTP请求上限已测试，失败不回退陈旧快照；认证、写操作和事件流待实现 |
