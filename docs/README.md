...
| 智能运动 | 第一阶段已实现、已测试 | Mock多轴、TIM2 compare调度、限位停机和遥测；TimeSync v1、四时间戳主机闭环、启动代次与安全换算门已完成纯软件测试，守护进程周期采样、跨板组事务与实体时序验收待实现 |
| TMC2209 | 第一阶段已实现、部分实测 | FLY-D5五路单线通信及五电机已实测，F103/G431待系统验收 |
| Studio | 构建阶段已实现 | 工程schema v2、冲突检查、I2C/SPI总线与设备图形编辑、前后端一致校验、Mock可视化、生成`.config`、32线程构建和产物归档；总线资源仍为Mock-only，GUI自动烧录回读待实现 |
...
| Runtime API | 只读竖切已实现、已测试 | 版本化REST快照、节点、资源、告警接口；默认通过严格的`remote-cli --json`读取toolbusd本地IPC，旧文本仅显式兼容；单次快照IPC/原生绑定、缓存并发、认证、写操作和事件流待实现 |
