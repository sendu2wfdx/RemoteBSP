...
| 智能运动 | 第一阶段已实现、已测试 | Mock多轴、TIM2 compare调度、限位停机和遥测；TimeSync v1、四时间戳主机闭环、启动代次、安全换算门及toolbusd有界周期采样已完成纯软件测试，跨板组事务与实体时序验收待实现 |
| TMC2209 | 第一阶段已实现、部分实测 | FLY-D5五路单线通信及五电机已实测，F103/G431待系统验收 |
| Studio | 构建阶段已实现 | 工程schema v2、冲突检查、I2C/SPI图形编辑、Mock可视化、`.config`与32线程构建；可生成确定性中文接线表、资源摘要、稳定清单和SHA-256资料包，总线资源仍为Mock-only，自动烧录回读待实现 |
...
| Runtime API | 只读竖切已实现、已测试 | 版本化REST接口及严格`remote-cli --json`；已有限时单飞缓存、明确新鲜度、资源查询并发和HTTP请求上限，失败不回退陈旧快照；单次一致快照IPC、认证、写操作和事件流待实现 |
