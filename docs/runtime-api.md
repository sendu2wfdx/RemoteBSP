...
受控网络中由 TLS 反向代理终止 HTTPS，不能把明文 HTTP 直接暴露到不可信网络。当前也可
通过 `--tls-baseline-config` 让 Runtime 在数字回环地址直接终止 HTTPS；启动前会验证版本化
基线、绑定地址、证书/私钥权限与配对关系，缺失或漂移即失败关闭。该入口仍禁止公网绑定，
详见 [Runtime TLS 部署基线](runtime-tls-deployment-baseline.md)。
...
并通过同时保留新旧两个密钥完成有限时间轮换。Runtime 会在每次认证前以
`device/inode/size/mtime/ctime` 指纹检查配置：写入方应在同一文件系统生成并校验完整临时
文件，再原子替换目标路径。新文件换入后的首个请求即使用新身份和权限，旧密钥随即失效，
无需重启服务。

热加载采用失败关闭：文件丢失、超限、JSON 损坏、版本或权限非法，以及读取期间再次发生
替换时，当前认证表立即不可用，所有受保护请求返回未认证；不会继续信任内存中的旧密钥。
修复并原子换入合法文件后可自动恢复。运维时建议先加入新密钥、确认新客户端可用，再原子
换入删除旧密钥的第二版；不要直接原地截断并逐段写入生产文件。
...
v3 在时钟记录后追加最多 128 项、每项固定 28 字节的 `bus_health`，响应头原保留字段
改为该数组的条目数。每项以节点和设备资源 ID 为稳定身份，提供最后明确远端事务状态、
toolbusd 单调时钟微秒值、当前连续失败及历史峰值。`Ok` 只清当前连续失败，未知状态不
更新状态；节点重连代次变化会清除旧项。IPC 与 Provider 均严格拒绝未知状态、重复或
悬空身份以及当前值大于峰值的记录，不把软件状态推断成实体总线波形。

Runtime Provider 对 v3 使用封闭字段集合，严格检查根信封、命令名、字段类型、数值
范围、固定枚举、流量类别顺序，以及响应中的节点/资源 ID 是否与请求相符。未知版本、
缺失或多余字段、非 UTF-8、超过 1 MiB、超时或非零退出均显式失败。版本升级应新增
解析器并经过明确协商，不能让旧版解析器猜测新字段语义。

v4 在响应头增加 GPIO 输入诊断返回条目数和已登记总数，并在尾部追加最多 128 项固定长度
对象记录。总数大于返回数时，CLI 明确输出 `gpio_input_diagnostics_truncated=true`，
Runtime 生成稳定容量告警，不能把截断误读为其余对象状态正常。
每项绑定节点、静态 GPIO 资源、运行期对象和引脚，分别携带事件队列水位/丢弃数及
可选的 EXTI mailbox/hint 计数。toolbusd 只投影当前节点代次内已成功订阅的对象；每个
对象独立读取，超时或坏载荷仅令该项 `status_valid=false`，不会丢弃同节点其他对象或
整份快照。解码器仍接受 v3，并把缺失的对象诊断解释为“该版本未提供”，而不是零计数。
Runtime 将这些记录置于节点 `runtime.gpio_input_diagnostics`，HTTP、SSE 和事件快照共享
同一完整投影；读取失败或任一丢弃计数非零会产生稳定、可去重的节点告警。

RuntimeSnapshot 请求自身仍是严格版本字段。新版 toolbusd 同时接受 v3/v4 请求，并保留
请求版本：v3 请求只返回 v3 固定头和既有字段，绝不夹带 GPIO 扩展；v4 请求返回 v4。
因此滚动升级可先更新 toolbusd，旧 v3 客户端仍可工作，再更新 lib/CLI/Runtime。反方向
没有“支持版本集合”协商，旧 toolbusd 会明确拒绝 v4；客户端不会根据本地化错误文本
自动重试，以免把协议损坏误判为可降级。若未来需要任意升级顺序，应新增独立能力查询或
结构化 `UnsupportedVersion`，不能伪装成透明回退。
...
Runtime 看板和 `overview/stream` SSE 会投影 HealthSnapshot IPC v2 的 `ipc` 状态：当前连接
水位、上限、峰值、累计值以及相对同一 toolbusd 实例上一份新样本的增量。容量拒绝、超限
帧、请求超时或线程创建失败出现正增量时生成可解释告警；下一份新样本增量归零后告警恢复。
当前水位达到上限时另有容量饱和告警。同一健康样本被 HTTP 和 SSE 重复读取不会重复消费
增量，toolbusd 实例变化则重新建立基线并把增量标为 `null`，不会把重启清零解释成恢复或
负增量。看板展示的是 toolbusd 应用层 Unix socket 指标，不代表系统 FD 或 CAN 总线状态。
...
内核测试和 6 项 HTTP 集成测试；本阶段 Runtime 全量 207 项软件回归已通过。数字只对应
当前测试清单，不包含实体板或生产文件系统真实掉电证据。TLS、反向代理信任边界、用户
目录和动态角色、控制审计密钥轮换与外部链头锚定、速率限制、
