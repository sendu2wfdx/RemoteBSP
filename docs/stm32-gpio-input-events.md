...
正式后端当前采用周期采样，没有启用 EXTI 捕获。以后若加入 EXTI，中断只能记录有界的
待采样标志或候选时间，仍由主循环完成去抖、会话检查、排队和发送；共享向量必须逐引脚
隔离，ISR 不得等待队列。

板卡能力目录 v3 已先加入独立 `exti.endpoints` 合同。端点必须引用允许输入的 GPIO，
`line` 必须等于引脚号，同一板卡每条 EXTI line 只能出现一次，并拒绝板级保留/AF 占用
引脚。当前端点状态均为 `planned`、`enabled=false`，不绑定 Kconfig 符号；因此默认固件不
编译 EXTI mailbox、不配置 NVIC，也不公布已实现能力。只有三板 ISR mailbox 和生成器均
落地后，端点才可改为 `implemented` 并绑定独立固件符号。
