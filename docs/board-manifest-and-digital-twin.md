...
当前描述不会修改 STM32 固件，也不会替代 `firmware/Kconfig`。Mock已经先接入
PWM和定时位流的精确引脚、定时器、通道、DMA及性能上限端点表；F072/F103/G431的
完整芯片引脚复用、Flash分区和安全电平尚未迁入该格式。等公共schema稳定后，再让
固件生成器和图形配置器读取同一描述包。
...
| `waveform_endpoints` | PWM/定时位流的引脚、定时器、通道、DMA和性能上限；当前为可选兼容字段 |
...
- 存在 `waveform_endpoints` 时，每个公开PWM/定时位流必须恰好拥有一个同类型、同实例
  的端点；PWM不声明DMA，定时位流必须声明DMA及最大位数和码率。
...
- 默认描述生成 16 GPIO、8 UART、2 ADC、2 Timer、3 STEPGEN、2 PWM、1 Storage、
  1定时位流和35份一一对应的能力合同；这不是
  框架固定轴数。
- 有运动轴的描述必须给出`motion_maximum_total_step_rate_hz`。它与每轴
  `maximum_step_rate_hz`共同限制执行器，并由`MOTION_CONTRACT`原样公开给主机；
  默认Mock使用200 kstep/s，FLY-D5描述使用保守的30 kstep/s。
...
- ADC 静态端点进入资源目录，数字孪生明确公布支持状态与资源 ID，并可执行可重复的原始码采样。
- Storage 静态端点进入资源目录，数字孪生明确公布支持状态与资源 ID；其确定性块内容
  与设备参数双页完全分离。
- Timer 静态端点进入资源目录，数字孪生明确公布支持状态与资源 ID，并使用确定性计数、捕获和单次等待结果。
- 运行时清单的PWM/定时位流会按端点表核对引脚、定时器、通道、DMA和能力上限；
  同定时器PWM只允许同频不同通道，定时位流独占定时器且DMA通道不得重复。
- TMC UART条目会生成固定40000 bit/s的逻辑UART资源，Linux仍负责TMC2209帧、CRC
  和寄存器语义；独立运动驱动绑定条目把逻辑轴关联到该UART端点，Mock不解析
  TMC寄存器，也不会把绑定误计为可操作资源。
...
- 芯片、封装、完整引脚复用、运动定时器静态占用和 ADC 映射；波形端点子集已在
  Mock schema v1中先行验证。
- SWD、晶振、CAN、USB、Bootloader 和硬件 ID 保留项。
- Flash/RAM、设备参数保留区、擦除粒度和安全默认电平。
- TMC2209设备级响应/故障注入、TMC SPI事务、输入采样和更完整的资源依赖图；
  运行时TMC单线UART逻辑资源实例化及轴—驱动引用已经完成。
...
Studio工程引用板级接口或通用引脚，并为每个逻辑GPIO保存稳定名称。板卡描述决定
“硬件允许什么”，工程生成的Kconfig决定“这台设备实际拿它做什么”。所有输出
必须具有明确的上电和故障安全
