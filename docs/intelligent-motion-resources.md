...
> 状态：Mock、STM32 compare执行内核、主机静态准入及时钟冻结的跨板事务骨架已实现；守护进程接线、实体压力标定与跨板同步实测待完成。
...
传输语义。它还不是完整速度曲线或 DDA 轨迹格式；Linux 轨迹规划器、跨板事务的
守护进程与实体固件接线、TMC 和 STM32 定时器产品化后端仍在后续。
