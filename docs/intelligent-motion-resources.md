...
- `CONFIG_MOTION_TIMER_HZ`配置STEP执行中断频率，F072/F103默认100 kHz，
  G431默认200 kHz；高、低电平各至少保持一个周期。
- 入队时依据定时频率和运动段持续时间检查每轴最大步数，超过可实现步频的
  运动段直接拒绝，不允许静默降速。
- 每个定时tick按绝对开始时间和段内比例执行多轴DDA，独立维护方向、使能、
  STEP电平、位置、累计步数和边沿数。
- 限位在执行tick中检查，触发后清空队列、关闭全部轴并锁存故障；IO回调失败
  同样进入安全停机。
- 连续段队列、最终段自动关闭使能、队列欠载、主动中止、故障清除和水位/
  停机指标已有主机单元测试。

FLY-D5/F072 已完成第一块实体板后端：Remote Core 运动命令直接接入队列，TIM2
提供 1MHz 单调时钟，TIM3 以 10kHz 调用`rbsp_motion_tick()`，并驱动五路可配置
STEP/DIR/EN。高、低电平各至少占一个 tick，因此当前单轴最高合法脉冲频率为 5kHz。
为适应 Cortex-M0，段内均匀分布改为相位累加式 DDA，定时中断不做
64 位除法。实板已完成五轴各 3200 STEP、6 秒末段自动停止，状态回报的五轴计数
均正确。
...
PA3、PA7、PB1 只是五个板级实例。仅当步进槽选择 TMC2209 驱动器时，内部后端才
会被链接和分配状态；波特率固定为 TMC2209 所需的 40000 bit/s。
...
- 是否编译 `STEPGEN`、`INPUT_SAMPLER`、`UART`、`SPI`、`TIMED_BITSTREAM`、
  `NVM_CONFIG` 等模块。
...
单线后端由已选择的 TMC2209 步进槽自动带入，不单独暴露为通用功能。
...
使用的临时出厂默认布局，必须迁移为下述运行时资源清单，不能作为最终接线真相。
...
6. 实现通用定时位流资源、Mock 数字孪生和 STM32 定时器 PWM + DMA 后端。
7. 使用 WS2812 验证定时位流，并进行 F103 极限测试，确定可承诺的小规模能力。
