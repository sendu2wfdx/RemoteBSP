...
(() => {
  ...
  function render(data) {
    ...
    const daemon=$("daemon"), health=data.toolbusd_health || {}, ipc=health.ipc || {}; daemon.replaceChildren(text("h2","toolbusd 健康"),chip(health.availability),chip(health.overall),kv({IPC当前连接:ipc.active_clients,IPC上限:ipc.maximum_clients,IPC峰值:ipc.peak_clients,IPC增量:ipc.deltas || {},健康峰值:(health.trend || {}).peaks || {}}),trend((health.trend || {}).samples),renderAlerts(health.threshold_alerts));
