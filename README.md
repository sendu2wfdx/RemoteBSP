...
| 智能步进 Mock | 板卡能力决定的多轴 STEP/DIR/EN 时间线、有界队列、绝对/自动排程、欠载/限位安全停机和状态遥测；跨板事务已接入 Mock Remote Core，PREPARE 无副作用、COMMIT 单次入队，普通停止及会话/租约失效均会阻止旧事务执行 |
| Mock MCU | 版本化板卡描述、Classical CAN/CAN-FD、多节点、GPIO、UART、PWM、定时位流、I2C/SPI 原子事务和运动执行，并可导出数字孪生状态 |
| RemoteBSP Studio | 本地中文 GUI 首版：板卡资源工程、冲突过滤、I2C/SPI 图形编辑、Mock 数字孪生、工程差异、`.config` 生成、32线程构建和产物归档；可生成接线资料包及确定性生产记录，严格区分设计、软件构建、烧录和硬件实测状态。自动烧录/回读尚未实现 |
...
接入 Mock Remote Core 和实际 Mock 运动队列，覆盖租约与普通停止的安全失效；主机
协调器仍未接入 `toolbusd` 主循环，STM32 也未实现，不能作为跨板实体同步完成的结论。
...
漂移及误差估计贯通到 Runtime API；这些是软件模型观测值，不代表硬件已达到相同精度。
