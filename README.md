...
仍由 Linux 处理。单帧正文由协议层同时在编码和解码处限制为 2022 字节/16176 bit，
不会生成超过协议包上限的载荷。Runtime 已用独立权限贯通 configure/frame/stop、远端
租约、持久操作账本、IPC/client/CLI、认证 HTTP 和 Mock 三进程闭环；帧正文不进入账本，
仅保存覆盖完整正文的摘要，未知提交会冻结对应资源作用域。三块板均已交叉编译，但新 DMA 波形后端尚未使用示波器和实体
...
后端；设备参数 Web 两阶段写入/恢复也已实现并默认关闭。实体烧录验收和参数区掉电实测仍未完成。
...
API 密钥文件现支持原子热轮换：新文件首个请求即切换身份与权限，旧密钥即时撤销；文件
丢失、损坏或读取竞态会失败关闭，合法文件恢复后自动恢复认证。非回环部署仍必须由受控
TLS 反向代理、密钥文件权限和限速补齐。
