...
- 已生成确定性中文接线表、资源占用摘要、稳定资源清单和校验文件，并提供有界、
  ...
  的离线 Ed25519 脱离签名与 CLI 验证；本机 UTC 明确保持非可信，只用于排序。下一步补
  签名者授权、公钥轮换/吊销、外部可信时间证明、权限边界和归档迁移；
...
- 非交互生产 CLI 已覆盖工程校验、构建、ST-Link/CAN Katapult 部署、批次生成/校验和
  ...
  时间；下一步增加批量烧号编排、签名者权限、公钥轮换/吊销和外部可信时间证明；
- PWM 已贯通 `toolbusd` Gate、daemon、IPC、`libremotebsp` client、CLI、独立持久账本和
  ...
  固定端点；浏览器和工程文件不持有密钥，也不直连 CAN/toolbusd。通用定时位流已完成
  ...
  冻结资源作用域。WS2812 Studio 操作页及定时位流/灯带实体波形验收仍待完成；
...
- GUI 已增加节点/资源下钻、趋势、峰值锁存与阈值告警；受控告警规则具备持久化、权限、
  两阶段修改、迟滞和去抖。下一步做规则运维体验和系统级资源耗尽验证；
- 明确区分估算值、软件测量值和硬件实测值。
