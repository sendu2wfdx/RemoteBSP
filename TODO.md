...
- 已完成故障脚本 replay v1 及显式 Mock 逻辑传输钩子：固定单调时间基、seed、事件序号、
  H2N/N2H 方向、节点代次和完整状态摘要，覆盖资源状态与 delay/drop/duplicate/reboot；
  下一步记录真实 `toolbusd` 会话并确定性重放，再接物理链路采集，不把逻辑回放冒充
  CAN 仲裁、USB transaction 或真实复位时长；
...
- Runtime 默认使用版本化 `remote-cli --json`，单次本地 IPC 快照 v2 已贯通节点时钟
  ...
  但 HTTP 仍不下发命令；`toolbusd` 已另行实现受信本机 GPIO 控制 IPC，要求显式登记
  daemon 绑定租约、静态合同核对和幂等写入，同资源串行而跨资源不互相阻塞；
  下一步增加 TLS 部署基线、密钥热撤销、持久审计完整性、跨重启事件存储/主动推送，
  并把 HTTP 身份/权限/审计安全映射到该 IPC；
