...
- 已生成确定性中文接线表、资源占用摘要、稳定资源清单和校验文件，并提供有界、
  ...
  边界和归档迁移；
- 已新增独立部署作业软件模块，能校验受保护构建记录/固件、生成无 shell 拼接的
  ST-Link 命令计划、执行有界重试并核对板型/工程/配置/固件四类身份；下一步把
  ST-Link、CAN Katapult 和 USB Katapult 的实际执行器接入 Studio；
- 接入实体身份读取适配器，烧录后回读固件版本、UUID、配置哈希并给出明确结果；
...
- G431 `ResourceStatus` 已接入 UART 真实 RX/TX 环形缓冲水位与溢出计数，以及
  ...
  修复并在实板验证 Busy -> Normal、对象 1 不复位重建为对象 2。下一步接入 Bus、
  STEPGEN、更多资源错误计数和完整 MCU 健康生产者；未覆盖字段仍不得把全零解释为
  实体健康；
