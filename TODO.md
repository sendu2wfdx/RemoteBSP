...
- 已完成故障脚本 replay v1：固定单调时间基、seed、事件序号、脚本/板卡身份和完整
  状态摘要，覆盖 GPIO/UART/运动限位/总线故障/节点在线状态；下一步增加统一传输钩子，
  再记录 CAN 延迟、丢包和真正重启，不把当前状态回放冒充链路回放；
- 后续记录真实 `toolbusd` 会话并确定性重放。
...
- Runtime 默认使用版本化 `remote-cli --json`，单次本地 IPC 快照 v2 已贯通节点时钟
  ...
  并在 `toolbusd` 内把租约所有权与真正设备命令放进同一原子准入点；
...
- 高速 Stream 合同及 H2N/N2H Mock 有界会话已完成，覆盖租约、序号、防重放、
  精确累计 ACK、信用、两阶段提交、背压、故障和旧缓冲隔离；下一步补双向会话，
  并把所选链路绑定到独立数据面，不通过 CAN 透明隧道全部数据；
