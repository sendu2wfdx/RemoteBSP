...
- 主机四时间戳模型、`TimeSync v1`、Mock `boot_epoch` 和同步管理器已接入
  `NodeRegistry`；下一步接入 `toolbusd` 有界周期调度、真实传输边界锁存和实体
  计数器 BSP，并验证可靠启动代次来源；
...
- Runtime 默认使用版本化 `remote-cli --json`，旧文本仅显式兼容；下一步用单次快照
  IPC 或结构化进程内绑定消除 N+1 本地调用，并增加快照缓存与有界并发；
