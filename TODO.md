...
- 跨板运动组的版本化线格式、时钟冻结、协调器、RequestManager 服务桥接、
  ...
  公共 Remote Core 参与者已经完成；下一步为三款板提供经过掉电/快速复位验证的可靠
  `motion_boot_epoch()`，补齐固件 STEPGEN 租约生命周期，并处理部分 COMMIT 已送达
  后的网络分区、尽力停止和运维恢复；
...
- Runtime 默认使用版本化 `remote-cli --json`，单次本地 IPC 快照 v2 已贯通节点时钟
  ...
  失败合并和明确新鲜度；API key 认证已实现，下一步增加角色授权、TLS 部署基线、
  事件流、写操作租约和结构化审计，并评估进程内绑定；
