...
配置、ELF/BIN/HEX/MAP、日志、Git/工具链信息和SHA-256，并完成三块正式板卡的
真实交叉编译。下一步：
...
- 跨板运动组的版本化线格式、时钟冻结、准备、提交、取消、超时与 Mock 参与者已经
  完成纯软件闭环；下一步接入 `toolbusd` 事件循环、Remote Core 和实体固件，并加入
  开始前许可租约，处理部分 COMMIT 已送达后的网络分区；
...
- Runtime 默认使用版本化 `remote-cli --json`，已增加单次本地 IPC 快照、单飞短缓存、
  失败合并、明确新鲜度和 HTTP 活动请求上限；新路径消除了每次刷新中的 N+1 进程与
  套接字连接，下一步评估进程内绑定并增加认证、事件流和写操作租约；
...
- 已完成 `I2C_BUS`/`I2C_DEVICE`、`SPI_BUS`/`SPI_DEVICE` 线协议、主机 API、严格
  ...
  Mock 清单；下一步生成固件静态表并实现 STM32 BSP；
