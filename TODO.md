...
- 已把独立部署作业接入非交互 CLI 的显式 `deploy-stlink` 命令：校验受保护构建记录/
  ...
  设备参数后端已增加版本化介质几何/擦写/提交能力契约，内部Flash和Mock外部EEPROM共用双页
  镜像、CRC、代次与最后提交语义；实体EEPROM驱动和电气验收仍未进行；
...
- Runtime 默认使用版本化 `remote-cli --json`，单次本地 IPC 快照 v2 已贯通节点时钟
  ...
  审计仍保持有界、非阻塞的进程内边界；
  认证 SSE 完整状态推送、慢客户端隔离及轮询降级已实现。ControlAuditJournal 已补齐
  真实临时文件系统与独立进程的软件演练，覆盖未承诺尾部裁剪、已承诺截断、链损坏、
  分段轮转、重启锁、轮转失败关闭和目录级隔离；它不替代目标介质的真实断电验收。
  下一步完成可靠 MCU boot generation 驱动的自动解冻和 TLS 部署基线；API 密钥已
