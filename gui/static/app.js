...
let catalog, board, axes = [], gpios = [], pwms = [], pixelStrips = [], state, demoRunning = false, lastPosition = new Map();
...
function renderResources(){renderPwmConfig();renderStripConfig();renderAxisTable();renderGpioTable();validate();$('#addPwm').disabled=pwms.length>=waveformTemplates('pwm').length;$('#addStrip').disabled=pixelStrips.length>=waveformTemplates('ws2812').length}
...
function renderAxisTable(){
  syncSharedEnable();
  $('#axisTable').innerHTML=axes.map((axis,index)=>{const shared=Number.isInteger(axis.enable_source);return `<div class="axis-row"><div class="axis-row-head"><b>轴 ${index+1}</b><button data-remove="${index}">移除</button></div><div class="pin-fields"><label>STEP<select data-axis="${index}" data-role="step">${pinOptions(axis.step,index,'step',false)}</select></label><label>DIR<select data-axis="${index}" data-role="dir">${pinOptions(axis.dir,index,'dir',false)}</select></label><label>DIR 方向<select data-dir-inverted="${index}"><option value="normal" ${!axis.dir_inverted?'selected':''}>正常</option><option value="inverted" ${axis.dir_inverted?'selected':''}>反相</option></select></label><label>EN 来源<select data-enable-source="${index}">${enableSourceOptions(index,axis.enable_source)}</select></label><label>EN<select data-enable-pin="${index}" ${shared?'disabled':''}>${shared?`<option>${axis.enable}</option>`:pinOptions(axis.enable,index,'enable',false)}</select></label><label>EN 极性<select data-enable-polarity="${index}" ${shared?'disabled':''}><option value="low" ${axis.enable_active_low?'selected':''}>低有效</option><option value="high" ${!axis.enable_active_low?'selected':''}>高有效</option></select></label><label>TMC UART<select data-axis="${index}" data-role="tmc_uart">${pinOptions(axis.tmc_uart,index,'tmc_uart',true)}</select></label><label>限位/DIAG<select data-axis="${index}" data-role="limit">${pinOptions(axis.limit,index,'limit',true)}</select></label></div></div>`}).join('')||'<p class="muted">尚未添加运动轴。</p>';
...
function validate(){
  const errors=[]; axes.forEach((axis,i)=>['step','dir','enable','tmc_uart'].forEach(role=>{if(!axis[role])errors.push(`轴 ${i+1} 缺少 ${roleNames[role]}`)}));
  ...
  const selections=[];axes.forEach(axis=>pinRoles.forEach(role=>axis[role]&&selections.push([axis[role],`运动 ${roleNames[role]}`])));gpios.forEach(gpio=>gpio.pin&&selections.push([gpio.pin,`GPIO ${gpio.name}`]));pwms.forEach(x=>x.pin&&selections.push([x.pin,`PWM ${x.name}`]));pixelStrips.forEach(x=>x.pin&&selections.push([x.pin,`WS2812 ${x.name}`]));const owners=new Map();selections.forEach(([pin,owner])=>{if(owners.has(pin))errors.push(`${pin} 同时用于 ${owners.get(pin)} 与 ${owner}`);else owners.set(pin,owner)});
  $('#validation').className=errors.length?'invalid':'valid';
  const shared=axes.filter(axis=>Number.isInteger(axis.enable_source)).length;
  $('#validation').innerHTML=errors.length?errors.map(x=>`<div>• ${x}</div>`).join(''):`✓ ${axes.length} 个轴、${gpios.length} 个数字 IO、${pwms.length} 路 PWM、${pixelStrips.length} 条灯带，引脚无冲突${shared?`；${shared} 个轴复用 EN`:''}`;
}
function loadBoard(id){
  board=catalog.boards.find(x=>x.id===id);axes=structuredClone(board.motion_defaults);gpios=structuredClone(board.gpio_defaults||[]);
...
async function init(){
  ...
  $('#boardSelect').onchange=e=>loadBoard(e.target.value);$('#addAxis').onclick=()=>{if(axes.length>=8)return;axes.push({step:null,dir:null,dir_inverted:false,enable:null,enable_source:null,enable_active_low:true,tmc_uart:null,limit:null});renderResources()};
  ...
  $('#exportConfig').onclick=exportManifest;$('#refresh').onclick=refresh;$('#demoMotion').onclick=()=>{demoRunning=!demoRunning;$('#demoMotion').textContent=demoRunning?'停止演示':'演示运动'};
