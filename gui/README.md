...
设备参数 Web 入口提供读取、备份以及默认关闭的两阶段受控写入/恢复；备份 v2 带规范
JSON 的 SHA-256 完整性摘要，恢复仍兼容既有 v1。服务启动时必须显式配置
`--toolbusd-socket`，写入口还必须单独启用。Web 与上述 CLI 复用同一后端，在副作用前
核对节点 UUID、参数 generation、当前维护状态和精确确认短语，并逐项使用 generation
CAS；写前审计意图和终态使用 HMAC 链记录，参数明文不进入审计。它仍不是批量烧号、
完整生产角色权限或实体 Flash 掉电验证结论。
